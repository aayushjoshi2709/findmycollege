...
export class CollegeService {
  constructor(
    @InjectRepository(CollegeEntity) private repo: Repository<CollegeEntity>,
  ...
  async addCollege(
    ...
    const college = this.repo.create({
      name: name,
      website: website,
      userid: userid,
      courses: courses,
    ...
    const createdCollege = await this.repo.save(college);
    return createdCollege;
  }
  async updateCollege(
    user: UserEntity,
    id: number,
    attrs: Partial<CollegeEntity>,
  ) {
    const college = await this.repo.findOne({ where: { id: id } });
    if (!college) throw new NotFoundException('user not found');
    if (user.id != college.userid)
  ...
  async getComments(collegeId: number) {
    ...
    await Promise.all(
      Object.keys(tempComment).map(async (idx, key) => {
        ...
        const user: UserEntity = await Promise.resolve(
          this.userRepo.findOne({ where: { id: uid } }),
        );
        const resuser = { id: user.id, username: user.username };
